    ...
    fetchInsightsByCategory(patientId, category).then(async function(ins) {
      ...
      if (clinEl) {
        if (!ins) {
          ...
          var parts = [];
          if (ins.interpretation) parts.push('<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--ink4);margin-bottom:6px">臨床解釈</div><div style="font-size:13px;color:var(--ink2);line-height:1.9;white-space:pre-wrap">' + ins.interpretation + '</div></div>');
          if (ins.recommendation) parts.push('<div><div style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--ink4);margin-bottom:6px">介入・推奨</div><div style="font-size:13px;color:var(--ink2);line-height:1.9;white-space:pre-wrap">' + ins.recommendation + '</div></div>');
          clinEl.innerHTML =
            '<div class="insight-box insight-box--blue">' +
              '<div class="insight-label">📋 臨床解釈・介入</div>' +
              (parts.length ? parts.join('') : '<div style="color:var(--ink4);font-size:13px">データなし</div>') +
            '</div>';
        }
      }
        ...
        if (!ins || !ins.patient_comment) {
          ...
          patEl.innerHTML =
            '<div class="insight-box insight-box--amber">' +
              '<div class="insight-label">🗒 患者向けコメント</div>' +
              '<div style="font-size:13px;color:var(--ink2);line-height:1.9;white-space:pre-wrap">' + ins.patient_comment + '</div>' +
            '</div>';
        }
      }
