...
function selectPatientScore(index, el) {
  ...
  var encodedCat = s.category.split('').map(function(c) { if (c === ' ') return '%20'; if (c === '/') return '%2F'; return c; }).join('');
  var encodedPid = (window._currentPatient && window._currentPatient.id) || '';
