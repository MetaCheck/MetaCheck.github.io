...
function renderPatientScores(scores) {
  ...
  grid.innerHTML = scores.map(function(s, i) {
    ...
    return '<div class="pt-score-row" onclick="selectPatientScore(' + i + ',this)" style="flex-direction:column;align-items:flex-start;gap:6px;padding:12px">' +
      '<div style="display:flex;align-items:center;justify-content:space-between;width:100%">' +
        '<span class="pt-score-name" style="font-size:11px">' + catName(s.category) + '</span>' +
        '<span class="rank-badge rank-' + rank + '" style="width:26px;height:26px;font-size:12px;flex-shrink:0">' + rank + '</span>' +
      '</div>' +
      '<div class="pt-score-bar-wrap" style="width:100%">' +
...
function selectPatientScore(index, el) {
  ...
  fetchCategoryResults(currentUser.id).then(function(results) {
    ...
    renderTrendChart(currentUser.id, s.category);
    loadMetaboliteTable(currentUser.id, s.category);
