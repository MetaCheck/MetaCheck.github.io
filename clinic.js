    ...
    if ((!status || status === 'trial') && !allPatients.length) {
      ...
      if (demoPatients.length) {
        ...
        if (mainScroll) {
          ...
          banner.innerHTML = '<span>📊 これはサンプルデータです。実際のデータは契約後に表示されます。</span>' +
            '<a href="https://cpc-bank.com/collab/#contact" target="_blank" style="background:var(--amber);color:#fff;padding:4px 12px;border-radius:6px;text-decoration:none;font-size:11px;font-weight:600;white-space:nowrap;margin-left:12px">お問い合わせ</a>';
...
async function selectClinicPatient(patientId) {
  ...
  document.getElementById('clinic-detail-meta').innerHTML =
    ...
    '<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">' +
      '<button onclick="showKitSentModal(\'' + patientId + '\')" style="padding:6px 14px;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer;font-family:\'DM Sans\',sans-serif;background:var(--foam);color:var(--emerald);border:1px solid var(--sage)">📦 発送連絡</button>' +
      '<button onclick="releaseScores(\'' + patientId + '\')" style="padding:6px 14px;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer;font-family:\'DM Sans\',sans-serif;background:var(--emerald);color:#fff;border:none">✅ 結果を公開</button>' +
    '</div>';
...
async function selectClinicCat(cardEl, patientId, category) {
  ...
  try {
    ...
    fetchInsightsByCategory(patientId, category).then(async function(ins) {
      ...
      if (clinEl) {
        if (!ins) {
          clinEl.innerHTML = '<div style="color:var(--ink4);font-size:13px;padding:12px">データなし</div>';
        } else {
          clinEl.innerHTML = '<div style="color:var(--ink4);font-size:12px;padding:8px">AIが解析中...</div>';
          const clinicalText = [ins.interpretation, ins.recommendation].filter(Boolean).join('\n\n');
          const summarized = await summarizeInsight(clinicalText, 'clinic');
          clinEl.innerHTML =
            '<div class="insight-box insight-box--blue">' +
              '<div class="insight-label">📋 臨床解釈・介入（AI要約）</div>' +
              '<div style="line-height:1.8">' + summarized + '</div>' +
              '<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">' +
                (ins.interpretation ? '<button onclick="showFullInsight(\'' + encodeURIComponent(ins.interpretation) + '\')" style="background:none;border:1px solid var(--border);border-radius:6px;padding:4px 10px;color:var(--emerald);font-size:11px;cursor:pointer">臨床解釈の全文</button>' : '') +
                (ins.recommendation ? '<button onclick="showFullInsight(\'' + encodeURIComponent(ins.recommendation) + '\')" style="background:none;border:1px solid var(--border);border-radius:6px;padding:4px 10px;color:var(--emerald);font-size:11px;cursor:pointer">介入・推奨の全文</button>' : '') +
              '</div>' +
            '</div>';
        }
      }
      ...
      if (patEl) {
        if (!ins || !ins.patient_comment) {
          patEl.innerHTML = '<div style="color:var(--ink4);font-size:13px;padding:12px">データなし</div>';
        } else {
          patEl.innerHTML = '<div style="color:var(--ink4);font-size:12px;padding:8px">AIが解析中...</div>';
          const summarized = await summarizeInsight(ins.patient_comment, 'patient');
          patEl.innerHTML =
            '<div class="insight-box insight-box--amber">' +
              '<div class="insight-label">🗒 患者向けコメント（AI要約）</div>' +
              '<div style="line-height:1.8">' + summarized + '</div>' +
              '<div style="margin-top:12px;text-align:right">' +
                '<button onclick="showFullInsight(\'' + encodeURIComponent(ins.patient_comment) + '\')" style="background:none;border:none;color:var(--emerald);font-size:12px;cursor:pointer;text-decoration:underline">全文を見る</button>' +
              '</div>' +
            '</div>';
        }
      }
...
function showKitSentModal(patientId) {
  ...
  overlay.innerHTML =
    '<div style="background:#fff;border-radius:16px;padding:28px;width:380px;box-shadow:0 8px 32px rgba(0,0,0,.2)">' +
      '<div style="font-size:16px;font-weight:600;color:var(--forest);margin-bottom:20px">📦 発送連絡</div>' +
      '<div style="font-size:12px;color:var(--ink4);margin-bottom:6px;letter-spacing:1px;text-transform:uppercase">解析ID</div>' +
      ...
      '<div style="font-size:12px;color:var(--ink4);margin-bottom:6px;letter-spacing:1px;text-transform:uppercase">投函日</div>' +
...
async function submitKitSent(patientId) {
  ...
  try {
    ...
    if (typeof showToast === 'function') showToast('発送連絡を記録しました', 'success');
  } catch(e) {
    if (typeof showToast === 'function') showToast('エラーが発生しました', 'error');
  }
}

// ─── 結果公開 ────────────────────────────
async function releaseScores(patientId) {
  if (!confirm('この患者の結果を公開しますか？\n公開後、患者がアプリで結果を確認できるようになります。')) return;
  ...
  try {
    ...
    if (typeof showToast === 'function') showToast('結果を公開しました', 'success');
  } catch(e) {
    if (typeof showToast === 'function') showToast('エラーが発生しました', 'error');
  }
}
