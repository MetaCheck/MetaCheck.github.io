    ...
    fetchInsightsByCategory(patientId, category).then(async function(ins) {
      ...
      if (clinEl) {
        if (!ins) {
          ...
          var clinicalText = [ins.interpretation, ins.recommendation].filter(Boolean).join('\n\n');
          var summarized = await summarizeInsight(clinicalText, 'clinic');
          var fullParts = [];
          if (ins.interpretation) fullParts.push('【臨床解釈】\n' + ins.interpretation);
          if (ins.recommendation) fullParts.push('【介入・推奨】\n' + ins.recommendation);
          var fullText = encodeURIComponent(fullParts.join('\n\n'));
          clinEl.innerHTML =
            '<div class="insight-box insight-box--blue">' +
              ...
              '<div style="margin-top:12px">' +
                '<button onclick="showFullInsight(\'' + fullText + '\')" style="background:none;border:1px solid var(--border);border-radius:6px;padding:4px 10px;color:var(--emerald);font-size:11px;cursor:pointer">全文を見る</button>' +
              '</div>' +
            '</div>';
        }
      }
      ...
      if (patEl) {
        if (!ins || !ins.patient_comment) {
          ...
          var summarizedPat = await summarizeInsight(ins.patient_comment, 'patient');
          patEl.innerHTML =
            '<div class="insight-box insight-box--amber">' +
              '<div class="insight-label">🗒 患者向けコメント（AI要約）</div>' +
              '<div style="line-height:1.8">' + summarizedPat + '</div>' +
