...
async function loadMetaboliteTable(patientIds, category) {
  ...
  try {
    ...
    // 異常値順（log2fc降順: ↑が上、↓が下）でソート
    // log2fcがない場合はsample_value - baselineで代替
    var sorted = compounds.slice().sort(function(a, b) {
      ...
      function getScore(f) {
        if (!f) return 0;
        if (f.log2fc != null) return Number(f.log2fc);
        if (f.sample_value != null && f.baseline != null && Number(f.baseline) > 0)
          return Number(f.sample_value) - Number(f.baseline);
        return 0;
      }
      return getScore(fb) - getScore(fa);
    });
