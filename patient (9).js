...
async function renderPatientPage(patient) {
  ...
  document.getElementById('patient-name-display').textContent = displayName + ' さん';
  document.getElementById('patient-display-id').textContent = '解析ID: ' + patient.id;
  document.getElementById('patient-display-name').textContent = displayName + ' さん';
      ...
      if (grid) {
        ...
        grid.innerHTML =
          '<div style="background:var(--foam);border:1px solid var(--sage);border-radius:12px;padding:28px;text-align:center;margin-bottom:16px">' +
            ...
            '<div style="font-size:15px;font-weight:600;color:var(--forest);margin-bottom:8px">結果を準備中です</div>' +
            '<div style="font-size:13px;color:var(--ink3);line-height:1.7">検査結果が届いています。<br>クリニックでの説明後に公開されます。<br>しばらくお待ちください。</div>' +
          '</div>';
      }
      return;
    }
...
function selectPatientScore(index, el) {
  ...
  dbSelect('category_results', 'category=eq.' + encoded + '&select=*&limit=1').then(function(crRows) {
    ...
    fetchInsightsByCategory(currentUser.id, s.category).then(async function(ins) {
      ...
      el2.innerHTML = '<div style="color:var(--ink4);font-size:12px;padding:8px">AIが解析中...</div>';
      ...
      el2.innerHTML =
        '<div class="insight-box insight-box--amber">' +
          ...
          '<div style="margin-top:12px;text-align:right">' +
            '<button onclick="showFullInsight(\'' + encodeURIComponent(ins.patient_comment) + '\')" style="background:none;border:none;color:var(--emerald);font-size:12px;cursor:pointer;text-decoration:underline">詳細を見る</button>' +
          '</div>' +
        '</div>';
    });
  });
  ...
  overlay.innerHTML =
    '<div style="background:#fff;border-radius:16px;padding:24px;max-width:600px;width:100%;max-height:80vh;overflow-y:auto">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">' +
        '<div style="font-size:14px;font-weight:600;color:var(--forest)">🗒 詳細な説明</div>' +
